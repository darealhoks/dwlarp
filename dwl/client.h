...
{
	/* Clip the surface to its content box (geom minus a border on every side).
	 * Must subtract 2*bw, not bw: c->geom spans the full window incl. both
	 * borders. With only one bw subtracted the clip ran a border-width past the
	 * content's right/bottom edge. For SSD/non-CSD clients wlroots' own
	 * min(clip, surface) clamp hid this, but CSD clients (librewolf/GTK, whose
	 * surface extends into invisible shadow margins) escaped the clamp, so the
	 * per-buffer corner rounding landed a border-width off and the bottom/right
	 * corners frayed into stray pixels. */
	*clip = (struct wlr_box){
		.x = 0,
		.y = 0,
