...
struct Client {
	...
	float opacity; /* content opacity: 1.0 focused, unfocus_opacity otherwise */
};
...
void
setbufopacity(struct wlr_scene_buffer *buffer, int sx, int sy, void *data)
{
	wlr_scene_buffer_set_opacity(buffer, *(float *)data);
}

/* Dim a client's content by walking its surface buffers. Focused windows are
 * fully opaque; unfocused ones drop to unfocus_opacity so focus stays legible
 * even with zero-width borders. Opacity lives on the scene_buffer nodes and so
 * survives content updates; resize() reapplies c->opacity for buffers that
 * appear later (e.g. subsurfaces). */
void
...
mapnotify(struct wl_listener *listener, void *data)
{
	 ...
	 * ring while transparent windows still show what's behind them. Kept
	 * ABOVE the surface so the rounded hole doubles as a corner mask: the
	 * opaque ring covers each corner triangle the surface would otherwise
	 * poke through square. Surface-buffer rounding alone can't round CSD
	 * clients (their buffer corners sit out in invisible margins that the
	 * geometry clip then squares off), so the mask is what makes corners
	 * round uniformly for every client, CSD or SSD. */
...
resize(Client *c, struct wlr_box geo, int interact)
{
	 ...
	 * A scene_shadow fills its whole box with solid shadow color and feathers
	 * outward; left unclipped that solid fill sits under the window and bleeds
	 * through transparent clients (the "inward shadow"). Clip out the window's
	 * own footprint so only the outward feather paints. Clip is in shadow-local
	 * coords — the -2 cancels the 2px drop so the hole lines up with the window. */
